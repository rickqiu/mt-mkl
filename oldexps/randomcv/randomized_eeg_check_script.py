...
kernels = [
    sorted([
        os.path.join(p, k) for k in os.listdir(p)
        if not k.startswith(p.split('/')[-1])
...
klc = model_selection.MultipleKernelRandomizedSearchCV(
    ...
    param_distributions={
        'lamda': uniform(loc=0.01, scale=1),
...
cv_results = cross_validate(
    klc, X_list, y_list, cv=StratifiedShuffleSplit(test_size=0.5, n_splits=50))
